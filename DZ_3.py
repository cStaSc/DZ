...
#############################
class Gaussian(SourcePlaneWave):
...
if __name__ == '__main__':
    ...
    layer_loss = 100 #координата начала потерь
    loss[:layer_loss] = 0.005 #начало поглащающего слоя слево
    ceze = (1.0 - loss) / (1.0 + loss) #массив
    cezh = W0 / (eps * (1.0 + loss)) #массим
    print(len(loss))
    # Усреднение коэффициентов на границе поглощающего слоя
    ceze[layer_loss] = (ceze[layer_loss - 1] + ceze[layer_loss + 1]) / 2
    cezh[layer_loss] = (cezh[layer_loss - 1] + cezh[layer_loss + 1]) / 2

    # Коэффициенты для расчета поля H
    chyh = ceze #согласование потерь в средах 
    chye = 1 / (W0 * (1 + loss))
    print(ceze)
    print(cezh)
    print(chyh)
    ...
    signal = GaussianModPlaneWave(500, 500, 300, 1)
    ...
    for t in range(1, maxTime):
       ...
       # Ez[1] в предыдущий момент времени
        ...
        #Граничные условия ABC для правой границы 
